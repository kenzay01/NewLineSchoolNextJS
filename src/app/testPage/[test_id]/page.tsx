...
import "../../styles/TestStartingPage.css";
import { TESTS, TestKeys, testTitles, test_descriptions } from '../../dict/test';
              ...
              <div className="test-starting-page-test-list">
                {testContent.map((item, index) => (
                  <TestItem
                    ...
                    isCorrect={answers[
                      testId
                    ]?.userAnswersCount.correct.includes(item.id)}
                    pickedAnswer={answers[testId]?.[item.id]}
                  />
                ))}
              </div>
            ) : null}
        {!checkedAnswers ? <div className="test-starting-page-test-container-form-btn-container">
          <button
            className="test-starting-page-test-container-form-btn"
            onClick={() => {
              isRegistrated ? testCompleted(testId) ? handleClickCheckAnswers() :  handleClickFinish() : handleClickStart();
            }}
          >
            {isRegistrated
              ? testCompleted(testId)
